...
class Portfolio:
    def __init__(self, 
        ...
        if (allocations is not None) and (tot_value is not None):
            raise RuntimeError("Refusing to create a portfolio with both allocations and total value." 
                               "Provide allocations only and the total value will be computed.")
        if (allocations is None) and (tot_value is None):
            raise RuntimeError("Cannot create a portfolio without `allocations` or (`tot_value`).")
    ...
    def _sync_weights(self):
        self.weights = np.array([self.current_prices[i] * self.allocations[i] / self.asset_value for i in range(self.num_assets)])
    ...
    def random_move(self):
        ...
        if first_2_second:
            if (
                ((num2 * price2 - num1 * price1) <= self.cash_value)
                and 
            ...
            if (
                ((num1 * price1 - num2 * price2) <= self.cash_value)
                and 
    ...
    def copy(self):
        return Portfolio(self.current_prices, 
                         cash_value=self.cash_value,
                         allocations=self.allocations.copy())


    def __str__(self):
        return (f"Portfolio with total value of {round(self.tot_value, ndigits=1)}"
